...
class Transformer(nn.Module):
    def __init__(
        ...
        for _ in range(depth):
            sa = Attention(dim, heads=heads,
                           dim_head=dim_head, dropout=dropout)
            ff = FeedForward(dim, mlp_dim, dropout=dropout)
            self.layers.append(
                nn.ModuleList(
                    [
                        PreNorm(dim, sa, norm=norm,
                                norm_cond_dim=norm_cond_dim),
                        PreNorm(dim, ff, norm=norm,
                                norm_cond_dim=norm_cond_dim),
                    ]
                )
            )
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, inp: torch.Tensor, *args, context=None, context_list=None):
        ...
        x = self.dropout(x)
        x += self.pos_embedding[:, :n]  # torch.Size([8, 1, 768]) torch.Size([2, 1, 2048])

        x = self.transformer(x, *args, context=context,  # context torch.Size([8, 64, 768])
