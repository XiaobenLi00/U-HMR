...
def build_position_encoding(cfg):
    # num_feats = cfg.MODEL.SMPL_HEAD.TRANSFORMER_DECODER.context_dim // 2
    if cfg.MODEL.SMPL_HEAD.POSITIONAL_ENCODING == 'SinePositionalEncoding3D':
        # TODO find a better way of exposing other arguments
        position_embedding = SinePositionalEncoding3D(context_dim=cfg.MODEL.SMPL_HEAD.TRANSFORMER_DECODER.context_dim, normalize=True)
    # elif cfg.MODEL.SMPL_HEAD.position_embedding in ('v3', 'learned'):
...
if __name__ == "__main__":
    position_embedding = SinePositionalEncoding3D(context_dim=512)
    # position_embedding = PositionEmbeddingSine(num_pos_feats=256)
    pos_embed = position_embedding(torch.randn(2, 4, 512, 8, 8))
