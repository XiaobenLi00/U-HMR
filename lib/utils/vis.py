...
CONNECTIVITY_DICT = {
    ...
    "h36m": [(3, 2), (2, 1), (1, 0), (6, 5), (5, 4), (4, 0), (0, 7), (7, 8), (8, 9), (9, 10), (8, 14), (14, 15), (15, 16), (8, 11), (11, 12), (12, 13)],
...
def visualize_2d_pose(images, keypoits_2ds, kind = 'h36m', size = 5):
    images = image_batch_to_numpy(images)
    images = denormalize_image(images).astype(np.uint8)
    n_cols = 2
    # n_rows = images.shape[0]
    n_rows = min(images.shape[0], 4 * 8)
    keypoits_2ds = to_numpy(keypoits_2ds)
    fig, axes = plt.subplots(ncols=n_cols, nrows=n_rows, figsize=(n_cols * size, n_rows * size))
    axes = axes.reshape(n_rows, n_cols)
    for row_i in range(n_rows):
        axes[row_i][0].imshow(images[row_i])
        axes[row_i][1].imshow(images[row_i])
        draw_2d_pose(keypoits_2ds[row_i], axes[row_i][1], kind = kind)
    fig.tight_layout()
    ...
    return fig_image.transpose(2, 0, 1)
...
def draw_2d_pose(keypoints, ax, kind='h36m', keypoints_mask=None, point_size=2, line_width=1, radius=None, colors=None):
    """
